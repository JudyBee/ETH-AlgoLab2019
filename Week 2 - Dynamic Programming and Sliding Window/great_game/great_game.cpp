...
int testcase() {
    int n, m; std::cin >> n >> m; // Reading the number of vertices and edges
    int r, b; std::cin >> r >> b; // Reading the starting positions of the meeples 
    std::vector<VI> graph(n + 1, VI()); // Adjacency list
    for (int i = 0; i < m; ++i) {
        int u, v; std::cin >> u >> v; // Reading an edge
        graph[u].push_back(v); // Store the edge 
    }
    VI mini(n + 1, MAX_N); VI maxi(n + 1, -1);
    ...
    std::cout << sherlock << " " << moriarty << std::endl;
    ...
    // the number of moves is not divisible by 2, as the sequence of moves is: // r --> b --> b --> r --> r --> b --> ...
    // s --> m --> s --> m --> s --> m --> ...
